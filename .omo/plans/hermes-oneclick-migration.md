...
- [ ] F1. Plan compliance audit - every todo's acceptance criteria met; `migrate` does import + cutover with dry-run/no-cutover; skills-dir + APPROVAL_MODE gaps closed.
...
- [ ] F2. Code quality review - `cargo clippy --all-targets -- -D warnings`, `cargo fmt --check`, no real destructive calls in tests, seam boundaries respected, typed errors (no unwrap/panic in migration paths).
  ...
  Recommended task executor category: unspecified-high
- [ ] F3. Real manual QA - build release; `cargo run -- migrate --dry-run` against real `~/.hermes` prints a coherent non-destructive plan; `cargo run -- run`/bare run still take the gateway path; full `cargo test` green in one run.
...
- [ ] F4. Scope fidelity - nothing outside scope changed (no state.db/memories/sessions migration, no plist deletion, no auto-cutover on startup); Must-NOT-Have honored.
